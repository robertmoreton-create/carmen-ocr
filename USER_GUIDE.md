...
4. Your Excel template files:
   - `Su - HSBC One #833 (PayMe).xlsx` (for HSBC)
   - `AMEX.xlsx` (for AMex)
...
Create a folder on your Desktop called `CarmenOCR`. Inside it, create these subfolders:
...
│   └── Docs/
│       └── Su - HSBC One #833 (PayMe).xlsx   ← your template
└── Amex/
...
|------|----------|-------------------|
| Settings | `Documents\CarmenOCR\carmen_settings.json` | Folder paths, template locations |
| Azure Key | `Documents\CarmenOCR\azure_config.json` | Your Azure endpoint and key |
